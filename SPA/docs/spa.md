...
- Feature autotag contract: every feature root in SPA partials and modal roots should include `data-autotag` using `feature-page-<page>-<name>` or `feature-modal-<name>` naming (for example, `feature-page-3-visitor-metadata` and `feature-modal-export-exclusion`). Runtime-generated feature roots created in JavaScript should set the same `data-autotag` at creation time so diagnostics can detect duplicates and missing tags consistently.
