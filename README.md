...
- `calculateMetadataTableValue()` in `SPA/js/3.js` targets the 7-day lookback span for each SubID/App ID + namespace row and stamps it with the literal text `hello world` after metadata queue runs, relying on per-cell data attributes so every table row stays addressable across the four metadata tables. Other lookback windows still hydrate from `metadataAggregations` via `getMetadataAggregationValue()`.
