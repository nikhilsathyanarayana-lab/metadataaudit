...
- `calculateMetadataTableValue()` in `SPA/js/3.js` now stamps every lookback span for each SubID/App ID + namespace row with the discovered metadata field names (sorted and comma-separated), relying on per-cell data attributes so every table row stays addressable across the four metadata tables. The helper accepts a direct target element so callers can update a single span without scanning the entire DOM, and aggregation totals remain recorded per lookback window so all three table columns stay stable after the queue finishes running.
