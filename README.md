...
- `lookbackDays()` in `SPA/API/metadata.js` normalizes SubID/App ID pairs, seeds `<subId>_<appId>_lookbackDays` counters with the current window size, and keeps those counters ready for decrementing as aggregations finish.
...
- `processAggregation()` in `SPA/API/metadata.js` receives aggregation responses from `requestMetadataDeepDive()`, logs SubID/app/timeseries context, tallies every visitor/account/custom/Salesforce field into value/count buckets so 7-day metadata can be inspected via `window.metadataAggregations`, and decrements the `<subId>_<appId>_lookbackDays` tracker by the processed window size when calls succeed.
- Lookback progress now logs milestone completions at 7-, 30-, and 180-day totals as `<subId>_<appId>_lookbackDays` counters decrease so console observers can tell when chunked API windows have covered those ranges.
