...
- `calculateMetadataTableValue()` in `SPA/js/3.js` targets the 7-day lookback span for each SubID/App ID + namespace row and fills it with the discovered metadata field names (sorted and comma-separated), relying on per-cell data attributes so every table row stays addressable across the four metadata tables. Other lookback windows still hydrate from `metadataAggregations` via `getMetadataAggregationValue()` and show value totals instead of field lists. The helper now accepts a direct target element so callers can update a single span without scanning the entire DOM. Aggregation totals are now recorded per lookback window so all three table columns remain stable after the queue finishes running.
...
- `processAggregation()` in `SPA/API/metadata.js` receives aggregation responses from `requestMetadataDeepDive()`, logs SubID/app/timeseries context, tallies every visitor/account/custom/Salesforce field into value/count buckets so recent metadata distributions can be inspected via `window.metadataAggregations`, and now stores a dedicated namespace bucket for each lookback window so 7/23/150-day counts persist independently. Derived 30-day and 180-day buckets are created in the same structure by merging the shorter windows (7 + 23) and the resulting 30-day view with the 150-day lookback, keeping the earliest `timeseriesStart` across merged buckets. Field name lists can be retrieved safely (without exposing value counts) via `getMetadataFields()` for each namespace.
...
- **SPA metadata aggregation summaries (`metadataAggregations`)**: View 3 stores SubID → App ID namespaces with timeseries start, per-window buckets, and value/count tallies for visitor/account/custom/Salesforce fields so recent metadata distributions can be reviewed from the console.
...
- `window.metadataQueue`
  - Context: SPA view 3 after metadata tables load. Builds a queue of 7/23/150-day metadata aggregation calls for each SubID + App ID pair on the page and exposes derived 30/180-day buckets (7 + 23, then 30 + 150) alongside the raw window data so the tables can present round-number totals.
