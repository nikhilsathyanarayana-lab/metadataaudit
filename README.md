...
- `calculateMetadataTableValue()` in `SPA/js/3.js` inspects `metadataAggregations` for each SubID/App ID row and namespace, summing value totals per lookback window so the Visitor, Account, Custom, and Salesforce tables can refresh their columns as aggregation results stream in.
