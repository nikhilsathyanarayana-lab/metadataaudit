...
- `processAggregation()` in `SPA/js/3.js` receives aggregation responses from `requestMetadataDeepDive()`, logs SubID/app/timeseries context, and tallies every visitor/account/custom/Salesforce field into value/count buckets so 7-day metadata can be inspected via `window.metadataAggregations`.
